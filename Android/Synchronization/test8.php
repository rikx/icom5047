...
$device_flag = 0;
if(isset($result["android_id"]) && !empty($result['android_id'])){
	$device_flag = 1;
}

if(!strcmp($_POST["sync_type"], "FULL")){
	$last_sync = date('Y-m-d H:i:s', strtotime('2000-01-01 00:00:00.0'));
}
...
$deleted_data = array();
if(isset($_POST["data"])){
	$local_Data = array_filter(json_decode($_POST["data"], true));
...
$report_selector = "";
if($user_type == "agent"){
	$report_selector = "AND creator_id = ".$user_id." ";
}
...
// TODO: filter by curent log user

$queries = array(
	'address' => "SELECT * FROM address WHERE last_update > '".$last_sync."' AND status != -1 OR address_id IN (SELECT address_id FROM location WHERE last_update > '".$last_sync."' AND location_id IN (SELECT location_id FROM report WHERE last_update > '".$last_sync."' AND status != -1))",
		...
		'flowchart' => "SELECT * FROM flowchart WHERE last_update > '".$last_sync."' AND status = 1 OR flowchart_id IN (SELECT flowchart_id FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector.")",
		'item' => "SELECT * FROM item WHERE flowchart_id IN (SELECT flowchart_id FROM flowchart WHERE last_update > '".$last_sync."' AND status = 1 OR flowchart_id IN (SELECT flowchart_id FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector.")) ",
		'option' => "SELECT option_id,parent_id,next_id,label FROM (SELECT item_id FROM item WHERE flowchart_id IN (SELECT flowchart_id FROM flowchart WHERE last_update > '".$last_sync."' AND status = 1 OR flowchart_id IN (SELECT flowchart_id FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector."))) AS questions INNER JOIN option AS answers ON questions.item_id=answers.parent_id",
		'location' => "SELECT * FROM location WHERE last_update > '".$last_sync."' AND status != -1 OR location_id IN (SELECT location_id FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector.")",
		'location_category' => "SELECT * FROM location_category WHERE last_update > '".$last_sync."'",//no status
		'report' => "SELECT * FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector."",// return only reports with active flowcharts
		'path' => "SELECT * FROM path WHERE last_update > '".$last_sync."' AND report_id IN (SELECT report_id FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector.")", // appointment must share status with report
		'appointments' => "SELECT * FROM appointments WHERE last_update > '".$last_sync."' AND status = 1 AND report_id IN (SELECT report_id FROM report WHERE last_update > '".$last_sync."' AND status != -1 ".$report_selector.")",
		'users_specialization' => "SELECT * FROM users_specialization WHERE last_update > '".$last_sync."'",//no status
		);
$queries_deleted = array(
	'address' => "SELECT address_id FROM address WHERE last_update > '".$last_sync."' AND status = -5",
	'person' => "SELECT person_id FROM person WHERE last_update > '".$last_sync."' AND status = -5",
		'users' => "SELECT users_id FROM users WHERE last_update > '".$last_sync."' AND status = -5", // TODO: only get de autorized users
		'flowchart' => "SELECT flowchart_id FROM flowchart WHERE last_update > '".$last_sync."' AND status = -5",
		'item' => "SELECT item_id FROM item WHERE flowchart_id IN (SELECT flowchart_id FROM flowchart WHERE last_update > '".$last_sync."' AND status = -5) ",
		'option' => "SELECT option_id FROM (SELECT item_id FROM item WHERE flowchart_id IN (SELECT flowchart_id FROM flowchart WHERE last_update > '".$last_sync."' AND status = -5)) AS questions INNER JOIN option AS answers ON questions.item_id=answers.parent_id",
		'location' => "SELECT location_id FROM location WHERE last_update > '".$last_sync."' AND status = -5",
		'report' => "SELECT report_id FROM report WHERE last_update > '".$last_sync."' AND status = -5",
		'path' => "SELECT  FROM report_id,option_id,sequence WHERE last_update > '".$last_sync."' AND report_id IN (SELECT flowchart_id FROM flowchart WHERE last_update > '".$last_sync."' AND status = -5)",
		'appointments' => "SELECT appointments_id FROM appointments WHERE last_update > '".$last_sync."' AND status = -5",
		);
...
{ 
	...
	if($device_flag){ // the divice is allredy in the db
			...
			if($user_type == "agent"){
				if(!strcmp($_POST["sync_type"], "FULL")){// sync full
					...
					$debug1 = "full";
						...
						if(isset($local_Data['appointments'])){
