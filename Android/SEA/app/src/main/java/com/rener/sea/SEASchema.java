...
//        dumm.setDummyData();
    ...
    public ArrayList<Flowchart> getAllFlowcharts() {
        SQLiteDatabase db = getReadableDatabase();
    ...
    public ArrayList<Report> getAllReports(long creator) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query(DBSchema.TABLE_REPORT, new String[]{DBSchema.REPORT_ID},
                DBSchema.REPORT_CREATOR_ID + "=?", new String[]{String.valueOf(creator)}, null, null, null, null);
        if ((cursor != null) && (cursor.getCount() > 0)) {
