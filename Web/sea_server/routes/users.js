 ...
 */
 ...
 router.get('/admin/citas', function(req, res, next) {
 	...
 	db.connect(req.conString, function(err, client, done) {
 		...
 		client.query("SELECT appointment_id, to_char(date, 'YYYY-MM-DD') AS date, to_char(appointments.time, 'HH24:MI') AS time, purpose, location_id, location.name AS location_name, report_id, agent_id, username \
