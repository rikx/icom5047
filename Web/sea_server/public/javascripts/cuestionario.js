        ...
        stop: function(event) {
          if ($(event.target).find('select').length == 0) {
            ...
            if(containsObject(this_item, elements_array)) {
              ...
              //replace($('#'+newState.attr('id')), states_array);
            } else {
              elements_array.push(this_item);
              states_array.push($('#'+newState.attr('id')));
            }
...
/*
  ...
  function replace(obj, list) {
    ...
    function replace_state(obj, list) {
    var i;
    for (i = 0; i < list.length; i++) {
  ...
  function recreate_graph(elements, connections){
    jsPlumb.ready(function() {
      ...
      for(var i=0; i<elements.length; i++){
        $('#container_plumbjs').append(elements[i]);
      ...
      for(var count = 0; count < connections.length; count++){
        ...
        for(var x = 0; x < elements.length; x++){
          ...
          if(this_element.attr('id') == this_connection.source){
            //source_item = document.getElementById(this_element.id);
            source_item = this_element.attr('id') ;
            console.log('Calling connect on source: '+this_element.attr('id'));
          } else if(this_element.attr('id')  == this_connection.target){
            //target_item = document.getElementById(this_element.id);
            target_item = this_element.attr('id') ;
            console.log('Calling connect on target: '+this_element.attr('id'));
          }
        }
        var common = {
